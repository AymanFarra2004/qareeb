        ...
        <div className="flex justify-between items-center h-20">
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`cursor-pointer px-4 py-2 rounded-full text-sm font-medium transition-all ${isActive
          ...
          <div className="hidden md:flex items-center space-x-3 rtl:space-x-reverse">
            ...
            <button
              ...
              className="cursor-pointer p-2.5 rounded-full bg-foreground/5 text-foreground hover:bg-foreground/10 transition-colors"
            ...
            {!isLoggedIn ? (
              <div className="flex items-center space-x-2 rtl:space-x-reverse border-s border-border/50 ps-3">
                ...
                <Link href="/sign-up" className="cursor-pointer px-5 py-2.5 text-sm font-semibold rounded-full bg-primary text-primary-foreground hover:opacity-90 shadow-lg shadow-primary/30 transition-all hover:scale-105 active:scale-95 bg-gradient-to-l from-blue-600 to-purple-600">
                  {t('auth.signUp')}
                </Link>
              </div>
            ) : (
              <div className="relative border-s border-border/50 ps-3">
                <button
                  ...
                  className="cursor-pointer flex items-center gap-2 p-2 rounded-full border border-border bg-background hover:bg-muted transition-all shadow-sm"
                >
                ...
                <AnimatePresence>
                  {isUserMenuOpen && (
                    ...
                    >
                      ...
                      <div className="py-2">
                        {isHubOwner && (
                          <Link href="/dashboard" className="cursor-pointer flex items-center gap-3 px-4 py-2 text-sm text-foreground hover:bg-muted transition-colors">
                            <LayoutDashboard className="h-4 w-4 text-primary" />
                            {t('userMenu.dashboard')}
                          </Link>
                        )}
                        {isAdmin && (
                          <Link href="/admin" className="cursor-pointer flex items-center gap-3 px-4 py-2 text-sm text-foreground hover:bg-muted transition-colors">
                        ...
                        <Link
                          ...
                          className={`cursor-pointer flex items-center gap-3 px-4 py-2 text-sm transition-colors ${pathname === '/profile'
                      ...
                      <div className="border-t border-border/50 pt-2">
                        <button
                          ...
                          className="cursor-pointer flex items-center gap-3 px-4 py-2 text-sm text-destructive hover:bg-destructive/10 w-full text-start transition-colors"
                        >
          ...
          <div className="flex md:hidden items-center space-x-1 rtl:space-x-reverse">
            ...
            */}
            ...
            <button
              ...
              className="cursor-pointer p-2 rounded-xl text-foreground bg-foreground/5 hover:bg-foreground/10 transition-colors"
            >
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    className={`cursor-pointer block px-4 py-3 rounded-xl text-base font-medium transition-colors ${isActive ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground"
                      }`}
                  >
                    {t(`nav.${link.labelKey}`)}
                  </Link>
              ...
              <div className="pt-4 mt-4 border-t border-border">
                {!isLoggedIn ? (
                  <div className="flex flex-col gap-3">
                    ...
                    <Link onClick={() => setIsMobileMenuOpen(false)} href="/sign-up" className="cursor-pointer w-full px-4 py-3 text-center text-sm font-semibold rounded-xl bg-primary text-primary-foreground shadow-md">
                      {t('auth.signUp')}
                    </Link>
                  </div>
                ) : (
                  <div className="space-y-3">
                    ...
                    {isHubOwner && (
                      <Link onClick={() => setIsMobileMenuOpen(false)} href="/dashboard" className="cursor-pointer flex items-center gap-3 px-4 py-3 rounded-xl bg-primary/10 text-primary font-medium">
                        <LayoutDashboard className="h-5 w-5" /> {t('userMenu.dashboard')}
                      </Link>
                    )}
                    {isAdmin && (
                      <Link onClick={() => setIsMobileMenuOpen(false)} href="/admin" className="cursor-pointer flex items-center gap-3 px-4 py-3 rounded-xl bg-destructive/10 text-destructive font-medium">
                    ...
                    <Link
                      onClick={() => setIsMobileMenuOpen(false)}
                      href="/profile"
                      className={`cursor-pointer flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-colors ${pathname === '/profile'
                    ...
                    <button
                      onClick={handleLogout}
                      className="cursor-pointer flex items-center gap-3 px-4 py-3 text-sm text-destructive hover:bg-destructive/10 rounded-xl w-full text-start transition-colors font-medium border border-transparent hover:border-destructive/20"
                    >
